...
import { Music2, User, Tag } from 'lucide-react';
...
export function BrowseSongs() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl tracking-tight font-bold text-zinc-900" style={{ fontFamily: 'Chivo, sans-serif' }}>
          Browse Songs
        </h2>
        <p className="text-sm text-zinc-500 mt-1" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 mb-2 flex items-center gap-2">
                <User className="w-3.5 h-3.5" /> Artist
              </label>
              <Select value={selectedArtist} onValueChange={setSelectedArtist}>
                <SelectTrigger data-testid="browse-artist-select" className="rounded-none">
                  <SelectValue>
                    {selectedArtist === 'all'
                      ? 'All artists'
            ...
            <div>
              ...
              <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                <SelectTrigger data-testid="browse-category-select" className="rounded-none">
                  <SelectValue>
          ...
          <div className="flex items-center justify-between border-t border-zinc-200 pt-4">
            <div className="text-sm text-zinc-600" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
              <span data-testid="browse-results-count" className="font-semibold text-[#002FA7]">{filteredSongs.length}</span>
              {' '}song{filteredSongs.length === 1 ? '' : 's'} found
            </div>
          </div>

          <ul className="border border-zinc-200 rounded-sm bg-white divide-y divide-zinc-100">
            {filteredSongs.length === 0 ? (
              <li className="py-12 text-center text-sm text-zinc-500" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
              ...
              filteredSongs.map((s, idx) => (
                <li
                  ...
                  className="px-5 py-3 flex items-center gap-4 hover:bg-zinc-50"
                >
                  <span className="text-xs font-mono text-zinc-400 w-10 text-right">{idx + 1}</span>
                  <Music2 className="w-4 h-4 text-[#002FA7] flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="text-base text-zinc-900 truncate" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
                      {kannadaSongName(s)}
                    </div>
                    <div className="text-xs text-zinc-500 truncate mt-0.5" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
